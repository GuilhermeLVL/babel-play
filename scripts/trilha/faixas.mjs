 ...
 *
 * O JSDoc existe para quem consome daqui do TypeScript: sem ele o `fora = {}` faz o tsc
 * inferir `{}` e qualquer `porNivel.A1` vira erro de índice implícito.
 *
 * @param {{ palavra: string, contagem: number }[]} palavras
 * @returns {Record<string, { palavra: string, contagem: number }[]>}
 */
...
export function coberturaDasFaixas(porNivel) {
  const massa = NIVEIS.reduce((s, n) => s + (porNivel[n] ?? []).reduce((t, p) => t + (Number(p.contagem) || 0), 0), 0)
  let acumulado = 0
  return NIVEIS.map((n) => {
