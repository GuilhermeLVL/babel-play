...
export interface ItemDaRodada {
  /* Opcional, como o `itemRef` do `ItemOutcome` de onde ele vem: quando o jogo não sabe
     identificar o item o campo fica AUSENTE, e aqui isso já significava chave/rótulo vazios. */
  itemRef?: string;
  cardId?: string | null;
  correct: boolean;
  attempts: number;
    ...
    <div className="card-panel p-5 space-y-4 max-w-xl w-full mx-auto my-auto max-h-[92vh] overflow-y-auto custom-scrollbar">
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <div>
          ...
          <div className="label-mono">acertos</div>
        </div>
        <div>
          <div className="text-[22px] font-extrabold text-ink tabular-nums">{tempo(tempoMs)}</div>
          <div className="label-mono">tempo</div>
        </div>
        <div>
          <div className="text-[22px] font-extrabold text-accent tabular-nums">+{xp}</div>
          <div className="label-mono">XP{combo && combo > 1 ? ` · combo ×${combo}` : ''}</div>
        </div>
      </div>
          ...
          {erradas.map((i) => (
            <div key={i.itemRef} className="flex items-center gap-2 text-[13px]">
              ...
              <span className="text-ink-muted flex-1 truncate">
                {i.back || <span className="opacity-60">sem tradução</span>}
              </span>
              {i.cefrLevel ? (
      ...
      {certas.length > 0 && (
        <div className="rounded-xl border border-border-subtle p-3">
          <button
            ...
            className="flex items-center gap-1.5 text-[12px] font-bold text-good cursor-pointer"
          >
          ...
          {verTodas && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {certas.map((i) => (
                <span key={i.itemRef} className="badge-tag ok">
                  {i.itemRef}
                </span>
              ))}
            </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {erradas.length > 0 && (
          <button
            onClick={() => aoRefazerErradas(erradas)}
            className="btn-solid px-3 py-2 text-[13px] flex items-center justify-center gap-1.5 cursor-pointer"
          >
            <RotateCw className="w-3.5 h-3.5" /> Refazer só{' '}
            {erradas.length === 1 ? 'a errada' : `as ${erradas.length} erradas`}
          </button>
        )}
        {/* Só aparece com desempenho bom E recorte suficiente — botão que falha ao clicar é pior que botão ausente. */}
        {podeSubirDificuldade && (
          <button
            onClick={aoSubirDificuldade}
            className="btn-outline px-3 py-2 text-[13px] flex items-center justify-center gap-1.5 cursor-pointer"
          >
        ...
        <button
          onClick={aoVoltar}
          className="btn-ink px-3 py-2 text-[13px] flex items-center justify-center gap-1.5 cursor-pointer"
        >
