...
import { type CartaoNoServidor, CARTOES_DA_SESSAO, listarCartoes, semearSessaoComCartoes } from './_fixtures'
...
test.describe('Revisao FSRS', () => {
  test('avaliar um cartao avanca o progresso e move o due no servidor', async ({ page }) => {
    ...
    const linhaDoExercicio = page.getByRole('button', {
      name: /Repetir o que você salvou|Revisão espaçada|Treino de memória/,
    })
    ...
    await expect(progresso, 'o indicador deveria avancar para o cartao 2').toHaveText(/^Cartão 2 de/, {
      timeout: 10_000,
    })
  ...
  /* D-006, modelo HÍBRIDO: depois de uma resposta CERTA a pessoa pode ajustar a nota entre
     Difícil, Bom e Fácil (a derivada vem marcada); depois de uma errada não há o que escolher. */
  test('depois de acertar, a nota pode ser ajustada para Fácil antes de avançar', async ({ page }) => {
    test.slow()
    await page.goto('/revisar')
    await expect(page.getByRole('main')).toBeVisible()
    await fecharSobreposicoes(page)
    ...
    const progresso = page.getByText(/^Cartão \d+ de \d+$/)
    await expect(progresso).toBeVisible({ timeout: 10_000 })

    const corpo = page.getByRole('main')
    let alvo: CartaoNoServidor | undefined
    /* O cartao e identificado pela TRADUCAO ("Tradução: rio"), que aparece nos dois formatos; na
       digitacao a palavra fica escondida na frase com lacuna, e o segundo cartao da fila costuma
       cair nesse formato. */
    for (const c of cartoes) {
      const traducao = CARTOES_DA_SESSAO.find((x) => x.word === c.word)?.back ?? c.back ?? ''
      if (!traducao) continue
      if (
        await corpo
          .getByText(new RegExp(`Tradução:\\s*${traducao}\\b`, 'i'))
          .first()
          .isVisible()
          .catch(() => false)
      ) {
    ...
    expect(alvo, 'nenhuma traducao da sessao esta no cartao aberto').toBeTruthy()

    const digitar = page.getByPlaceholder('Digite a palavra...')
    if (await digitar.isVisible().catch(() => false)) {
      await digitar.fill(alvo!.word)
      await digitar.press('Enter')
    } else {
      await clicarRobusto(page, corpo.getByRole('button', { name: alvo!.word, exact: true }))
    ...
    const notas = page.getByRole('radiogroup', { name: 'Nota da revisão' })
    await expect(notas, 'depois de acertar, as tres notas aparecem').toBeVisible()
    await expect(notas.getByRole('radio', { checked: true })).toHaveText(/Bom/)
    await clicarRobusto(page, notas.getByRole('radio', { name: /Fácil/ }))
    await expect(notas.getByRole('radio', { checked: true })).toHaveText(/Fácil/)
    await clicarRobusto(page, page.getByRole('button', { name: 'Avançar' }))
    await expect(progresso, 'a escolha nao trava o avanco').toHaveText(/^Cartão 2 de/, { timeout: 10_000 })
  })
})
