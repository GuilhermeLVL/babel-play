...
import { CabecalhoDeTela, Vazio } from '../ui';
  ...
  return (
    <div className="flex-1 overflow-y-auto w-full bg-canvas">
      <div className="p-6 md:p-10 max-w-6xl mx-auto w-full">
        {/* Cabeçalho — a linguagem muda por perfil; a estrutura (CabecalhoDeTela, redesign v3), não. */}
        <CabecalhoDeTela
          kicker={
            <>
              {ageProfile === 'kids' ? (
        ...
        {metrics && metrics.dueToday > 0 && (
          <section className="card-panel bg-surface border-accent/40 p-5 md:p-6 mb-8 flex flex-col sm:flex-row sm:items-center gap-5">
            ...
            >
              ...
              <span className="text-[10px] text-accent-ink mt-0.5">{tp(agora, 'palavra', 'palavras')}</span>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-start">
                ...
                <span className="text-[11.5px] text-ink-muted">
                  {t('Retenção, tempo de estudo e evolução semanal')}
                </span>
              </div>
            </div>
    ...
    >
      <div className={isSenior ? 'flex items-start gap-4 flex-1 min-w-0' : ''}>
        ...
        <div className="min-w-0">
          ...
          {status && (
            <p
              className={`mt-2 font-bold ${isSenior ? 'text-sm' : 'text-xs'} ${complete ? 'text-good-ink' : 'text-ink'}`}
            >
              {status}
            </p>
          )}
