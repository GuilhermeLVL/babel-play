...
export function useExameDePalavra(cartoes: VocabCard[], cartoesVotantes: VocabCard[] = cartoes): ExameDePalavra {
  ...
  const falar = (word: string) => {
    ...
    // `langPairOf().src` já é string; o `|| undefined` era resquício de quando `lang` era opcional.
    ttsSpeak(word, { lang, rate: velocidade });
  };
  ...
  const examinar = async (word: string, sentence?: string) => {
    ...
    setPalavraExaminada((prev) =>
      prev && prev.word === word ? { ...prev, translation: vocab.translation, mtEngine: vocab.mtEngine } : prev,
    );
  };
