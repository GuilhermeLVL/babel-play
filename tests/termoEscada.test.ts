...
function card(over: Partial<VocabCard> & { id: string; word: string }): VocabCard {
  return {
    // `id` e `word` não aparecem aqui: o `...over` no fim já os traz (são obrigatórios no
    // parâmetro) e sobrescreveria qualquer valor escrito antes.
...
describe('montagem dos degraus', () => {
  const palavras = Array.from({ length: 7 }, (_, i) => ({
    resposta: `P${i}`,
    palavra: `p${i}`,
    pista: 'x',
    lang: 'en',
  ...
  it('cada degrau ganha a folga de tentativas do seu tamanho', () => {
    const folgas = montarEscada(palavras, planoDaEscada(7)).map((g) => TENTATIVAS_POR_MODO[modoDeTabuleiros(g.length)])
