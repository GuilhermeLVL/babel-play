...
async function economia(s: AppDeTeste, userId: string): Promise<{ nivel: number; ganhas: number; gastas: number; saldo: number }> {
  const { asUserId } = await s.load('../../server/lib/authContext')
...
describe('seeds por HTTP (modo self-host)', () => {
  ...
  it('GET /api/metrics/profile devolve o perfil com a forma conhecida, e o saldo e o do razao', async () => {
    ...
    await expect(JSON.stringify({ status: r.status, forma: forma(corpo) }, null, 2)).toMatchFileSnapshot('__snapshots__/get.metrics.profile.seeds.json')
  })

  it('POST /api/metrics/seeds/gastar com item real ao preco do catalogo: 200, e o saldo cai o preco', async () => {
    const r = await s.post('/api/metrics/seeds/gastar', { spendId: `compra-${ITEM.id}`, amount: ITEM.precoSeeds, reason: `loja:${ITEM.id}` })
    expect(r.status).toBe(200)
    ...
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.gastar.json')
  ...
  it('o MESMO spendId de novo e idempotente: 200 com jaExistia=true, sem cobrar de novo', async () => {
    const r = await s.post('/api/metrics/seeds/gastar', { spendId: `compra-${ITEM.id}`, amount: ITEM.precoSeeds, reason: `loja:${ITEM.id}` })
    expect(r.status).toBe(200)
    expect(await r.clone().json()).toMatchObject({ jaExistia: true, gasto: ITEM.precoSeeds, seedsGastas: ITEM.precoSeeds })
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.gastar.repetido.json')
  ...
  it('preco divergente do catalogo: 400 preco_divergente com o preco certo em detalhes', async () => {
    const r = await s.post('/api/metrics/seeds/gastar', { spendId: 'compra-barata-01', amount: 1, reason: `loja:${CARO.id}` })
    expect(r.status).toBe(400)
    expect(await r.clone().json()).toMatchObject({ code: 'preco_divergente', detalhes: { preco: CARO.precoSeeds } })
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.gastar.preco-divergente.json')
  })

  it('preco certo mas saldo que nao paga: 402 saldo_insuficiente dizendo quanto falta', async () => {
    const r = await s.post('/api/metrics/seeds/gastar', { spendId: `compra-${CARO.id}`, amount: CARO.precoSeeds, reason: `loja:${CARO.id}` })
    ...
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.gastar.saldo-insuficiente.json')
    expect((await economia(s, DONO)).gastas).toBe(ITEM.precoSeeds)
  })

  it('motivo desconhecido: 400 com texto e SEM code', async () => {
    const r = await s.post('/api/metrics/seeds/gastar', { spendId: 'motivo-estranho-01', amount: 10, reason: 'qualquer-coisa' })
    expect(r.status).toBe(400)
    // caracterizacao: comportamento atual, nao desejado — a recusa de `autorizarGasto` sai como
    // `{ error }` cru, sem `code`, enquanto as outras recusas da mesma rota usam o envelope com code
    expect(await r.clone().json()).toEqual({ error: 'motivo desconhecido: qualquer-coisa' })
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.gastar.motivo-desconhecido.json')
  })

  it('item exclusivo de conquista nunca entra pela compra: 400', async () => {
    const r = await s.post('/api/metrics/seeds/gastar', { spendId: 'compra-aurora-01', amount: 10, reason: 'loja:tema-aurora' })
    expect(r.status).toBe(400)
  ...
  it('POST /api/metrics/seeds/creditar com conquista conferivel NAO cumprida: 400 conquista_nao_cumprida', async () => {
    ...
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.creditar.nao-cumprida.json')
    const perfil = await (await s.get('/api/metrics/profile')).json()
  ...
  it('creditar com conquista conferivel CUMPRIDA: 200 com o valor da regra, e o reenvio e idempotente', async () => {
    ...
    const r1 = await s.post('/api/metrics/seeds/creditar', { creditoId: 'conquista-sem-erro', amount: 9_999, xp: 9_999 })
    expect(r1.status).toBe(200)
    expect(await r1.clone().json()).toMatchObject({ jaExistia: false, seedsCreditadas: 20, xpCreditado: 30 })
    await expect(JSON.stringify(await resposta(r1), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.creditar.json')
  ...
  it('creditoId desconhecido: 400 credito_desconhecido', async () => {
    ...
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.seeds.creditar.desconhecido.json')
  })
  ...
  it('POST /api/metrics/presenca {} marca o dia do servidor; repetir no mesmo dia e jaExistia=true', async () => {
    ...
    await expect(JSON.stringify(await resposta(r1), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.presenca.json')
  ...
  it('presenca com dia 30 dias no passado: 400, sem gravar', async () => {
    ...
    expect(r.status).toBe(400)
    // caracterizacao: a recusa de janela sai como `{ error }` cru, sem `code`
    expect(await r.clone().json()).toEqual({ error: 'dia fora da janela aceitável' })
    await expect(JSON.stringify(await resposta(r), null, 2)).toMatchFileSnapshot('__snapshots__/post.metrics.presenca.fora-da-janela.json')
