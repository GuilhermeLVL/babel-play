...
const CHAMADOR_EXTERNO = {
  'DELETE /api/ai/credentials/:id': 'LACUNA DE PRODUTO: `src/data/api.ts` tem `listCredentials` e `createCredential`, e nenhuma função de apagar — quem cola uma chave de IA não consegue removê-la pela interface. O servidor está pronto e coberto (`tests/caracterizacao/ia.test.ts`: 200 e 404). Falta o botão',
  'GET /api/vocab/distribuicao-dificuldade': 'LACUNA DE PRODUTO: a distribuição por faixa de dificuldade não tem tela. O dado passou a existir quando `recalcularDificuldade` ganhou gatilho na rodada (change `schema-sem-tabela-orfa`); antes disso `difficulty_score` era NULL em 100% dos cartões e a rota não teria o que mostrar',
  'GET /api/vocab/:id/ocorrencias': 'LACUNA DE PRODUTO: a linha do tempo de "onde eu vi esta palavra" existe no servidor e no repositório (`vocabRepo.ocorrencias`, coberto por `z4-fio-completo`), e nenhuma tela a abre',
  'GET /api/anki/imports/:id': 'a importação de hoje é síncrona: o cliente recebe o resumo na resposta do POST e não tem o que acompanhar. O ledger existe para a importação assíncrona que as changes `motor-anki-*` planejam',
  'POST /api/ai/llm/chat/completions': 'o adapter do navegador (`src/gateway/adapters/openaiCompatible.ts:41`) fala direto com o provedor, montando `baseUrl + /chat/completions` com a URL DELE. Esta rota é o caminho que usa o segredo guardado e cifrado no servidor (`x-credential-id`) — o navegador não tem a chave para fazer o mesmo. Achado A52 da auditoria de 2026-09-07, mantido pela mesma razão que está escrita em `tests/contratos/rotas-espelhadas.test.ts`',
  'GET /api/sessions/:id/capa': 'a Biblioteca renderiza `rec.imageUrl` direto (`Library.tsx:729`), que é a `data:` URI guardada em `meta`. Esta rota serve a mesma capa como binário, para a lista não carregar megabytes de base64 — a otimização existe e ninguém a chamou ainda',
  ...
  'POST /api/admin/armazenamento/reconciliar': 'cron externo (documentado em `docs/deploy.md`) — a reconciliação oportunista no `/entitlements` cobre o resto',
...
function rotasDoServidor() {
  ...
  for (const [arquivo, prefixo] of Object.entries(PREFIXOS)) {
    for (const m of readFileSync(arquivo, 'utf8').matchAll(/\b\w+\.(get|post|patch|put|delete)\(\s*['"]([^'"]+)['"]/g)) {
  ...
  for (const m of billing.matchAll(/\b(\w+)\.(get|post|patch|put|delete)\(\s*['"]([^'"]+)['"]/g)) {
    ...
    rotas.push({ metodo: m[2].toUpperCase(), caminho: juntar(webhook ? '/api/billing/webhook/asaas' : '/api/billing', m[3]), arquivo: 'server/routes/billing.ts' })
  }
...
function padraoDaRota(caminho) {
  const partes = caminho.split('/').filter(Boolean).map((seg) => (seg.startsWith(':') ? '[^/`\'"\\s]+' : seg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
...
const tabela = rotas.map((r) => {
  ...
  return { ...r, chave, produto, soTeste: produto.length === 0 && chamadores.length > 0, motivo: CHAMADOR_EXTERNO[chave] }
})
...
if (process.argv.includes('--tabela')) {
  ...
  for (const l of tabela) {
    ...
    console.log(`| ${l.chave} | ${l.produto.slice(0, 3).join(', ') || '—'} | ${estado}${l.motivo ? ` — ${l.motivo}` : ''} |`)
  }
}
